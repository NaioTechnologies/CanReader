...
public class VerinCanFrameTest extends
	...
	public void testParseRequetePosition() {
		...
		assertEquals(canParser.getVerincanframe().getRequetePosition()
				.toString(), "50");
	}

	public void testParseRetourPosition() {
		canParser.parseOneFrame("(1215.1251) can0 401 [1] 64 00");
		assertEquals(canParser.getVerincanframe().getRetourPosition()
				.toString(), "100");
	}

	public void testLectureODO() {
		canParser.parseOneFrame("(1215.1251) can0 403 [1] 0F");
		assertEquals(canParser.getVerincanframe().getLectureODO().toString(),
				"15");
	}

	public void testVersionSoft() {
		canParser.parseOneFrame("(1215.1251) can0 405 [1] 01 02");
		assertEquals(canParser.getVerincanframe().getVersionMaj().toString(),
				"1");
		assertEquals(canParser.getVerincanframe().getVersionMin().toString(),
				"2");
	}

	public void testTension12v33v5vAndFlag() {
		canParser
				.parseOneFrame("(1215.1251) can0 406 [1] 01 02 03 04 05 06 02");
		...
		assertEquals(canParser.getVerincanframe().getFlagSortie().toString(),
				"2");
	}
