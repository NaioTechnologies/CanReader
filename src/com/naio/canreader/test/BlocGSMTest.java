...
public class BlocGSMTest extends ActivityInstrumentationTestCase2<MainActivity> {
	...
	public void textView_connection() {
		final String expected = mActivity
				.getString(R.string.global_text_not_connected);
	...
	public void dialogSendSms(Solo han) {
		...
		getInstrumentation().waitForIdleSync();
		assertTrue("Could not find the send a sms dialog!",
				han.searchText(mActivity.getString(R.string.sms_title)));
	}

	public void dialogCustom(Solo han) {
		han.clickOnButton(mActivity.getString(R.string.gsm_button_custom));
		getInstrumentation().waitForIdleSync();
		assertTrue("Could not find the custom dialog!",
				han.searchText(mActivity.getString(R.string.at_title)));
	}

	private void dialogPin(Solo han) {
		han.clickOnButton(mActivity.getString(R.string.gsm_button_pin));
		getInstrumentation().waitForIdleSync();
		assertTrue("Could not find the pin dialog!",
				han.searchText(mActivity.getString(R.string.pin_title)));
	}
